...
class IbanControllerTest {
        ...
        void testValidateIban_ValidIbanWithBankDetails() throws Exception {
                // Arrange
                BankDetails bankDetails = new BankDetails("DE445001", "Test Bank", "DE12345", "Test Branch",
                                "Test Address",
        ...
        void testValidateIban_InvalidIbanLength() throws Exception {
                // Arrange
                when(ibanService.validateIban(any(String.class)))
                                .thenThrow(new InvalidIbanLengthException(
        ...
        void testValidateIban_InvalidIbanFormat() throws Exception {
                // Arrange
                when(ibanService.validateIban(any(String.class)))
                                .thenThrow(new InvalidIbanFormatException(
                                                IbanValidationErrorEnum.INVALID_FORMAT.getMessage()));
                ...
                mockMvc.perform(post("/api/iban/validate")
                                ...
                                .andExpect(content().json(
                                                "{\"status\":\"error\",\"errorType\":\""
                                                                + IbanValidationErrorEnum.INVALID_FORMAT.name()
                                                                + "\",\"message\":\""
                                                                + IbanValidationErrorEnum.INVALID_FORMAT.getMessage()
                                                                + "\"}"));
        }
